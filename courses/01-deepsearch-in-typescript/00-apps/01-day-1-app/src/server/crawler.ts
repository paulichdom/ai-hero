...
const extractArticleText = (html: string): string => {
  ...
  for (const selector of articleSelectors) {
    ...
    if (element.length) {
      const markdown = turndownService.turndown(element.html() || "");
      if (markdown.trim()) {
  ...
  const fallback = turndownService.turndown($("body").html() || "");
...
export const crawlMultipleUrls = async (
  ...
  const results: BulkCrawlResult[] = await Promise.all(
    urls.map(async (url) => ({
      url,
