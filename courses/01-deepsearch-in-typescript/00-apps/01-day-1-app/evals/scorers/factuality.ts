...
const getQuestionFromMessages = (messages: Message[]) => {
  const lastUserMessage = [...messages]
    .reverse()
    .find((message) => message.role === "user");

  if (!lastUserMessage) {
    throw new Error("Factuality scorer requires at least one user message.");
  }
...
export const Factuality = createScorer<Message[], string, string>({
  ...
  scorer: async ({ input, expected, output }) => {
    if (!expected) {
      throw new Error("Factuality scorer requires an expected answer.");
    }
